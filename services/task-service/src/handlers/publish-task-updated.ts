...
export async function publishTaskUpdatedEvent(
  taskData: TaskUpdatedEventData,
  correlationId?: string
): Promise<void> {
  const log = createLogger(correlationId);

  try {
    const event: CloudEvent<TaskUpdatedEventData> = {
      specversion: '1.0',
    ...
    log.info('TaskUpdated event published', {
      eventId: event.id,
    ...
    log.error('Failed to publish TaskUpdated event', { error, taskData });
    throw error;
  }
}
